...
import time
import threading
from gpio_config import *  # Importiere die bestehende GPIO-Konfiguration
...
# Variable für die gemessene Entfernung
current_distance = 150  # in cm
current_angle_rad = math.pi/2  # Fester Winkel (90 Grad) in Radianten
detected_point = None

# Flag für den Sensormessungsthread
running = True
...
def measure_distance():
    ...
    # Begrenze die Entfernung auf den maximalen Anzeigebereich
...
# Thread-Funktion für kontinuierliche Messungen
def sensor_thread():
    ...
    while running:
        if scan_active:
            ...
            # LED-Blinksequenz je nach Entfernung
            if current_distance < 50:  # Nahe Objekte
                GPIO.output(LED_1, GPIO.HIGH)
                GPIO.output(LED_2, GPIO.LOW)
                GPIO.output(LED_3, GPIO.LOW)
            elif current_distance < 150:  # Mittlere Entfernung
                GPIO.output(LED_1, GPIO.LOW)
                GPIO.output(LED_2, GPIO.HIGH)
                GPIO.output(LED_3, GPIO.LOW)
            else:  # Weite Entfernung oder kein Objekt erkannt
                GPIO.output(LED_1, GPIO.LOW)
                GPIO.output(LED_2, GPIO.LOW)
                GPIO.output(LED_3, GPIO.HIGH)
                
        else:
            # Alle LEDs ausschalten, wenn nicht aktiv
            GPIO.output(LED_1, GPIO.LOW)
            GPIO.output(LED_2, GPIO.LOW)
            GPIO.output(LED_3, GPIO.LOW)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    if scan_active:
                        waves = [[0, 255]]
                        led_start()  # Starte die LED-Sequenz aus gpio_config
        ...
        if scan_active:
            ...
            for wave in waves:
                ...
                if abs(wave_radius - current_distance * distance_scale) < wave_speed and alpha > 150:
                    # Erkennungswert in 90° Winkel (geradeaus)
                    detected_point = (
